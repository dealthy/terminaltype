...
from var import CRUCIALVARIABLES

#global parameters used in test
class param():
	letter = []
	keys = [" "]
	endvalue = 0
...
#randomize words using shuf terminal command
def wordRad():
	...
	for i in range(CRUCIALVARIABLES.genmaxword):
		word = ""
		temp = os.popen("shuf -n 1 " + str(CRUCIALVARIABLES.dirpath) + "/words.txt").read()
